...
// 申请localStorage空间
localStorage.ingData == null &&
  localStorage.setItem('ingData', '提醒我定闹钟,明天带伞')
localStorage.doneData == null &&
  localStorage.setItem('doneData', '今晚整理衣物')

// 双区初始化
let ingData = ref(
  localStorage.ingData == '' ? [] : localStorage.ingData.split(',')
)
let doneData = ref(
  localStorage.doneData == '' ? [] : localStorage.doneData.split(',')
)
// localStorage.clear()
      ...
      <div class="done">
        ...
        <ul>
          <li v-for="(todo, index) in doneData" :key="index">
            <input
              type="checkbox"
              @click="zoneSwitch($event, doneData)"
              checked
            />
            <p>{{ todo }}</p>
